...
int main(int argc, char** argv) {
    std::cout << "=== Perception Pipeline ===\n"
              ...
              << "  c: corners | f: optical flow | s: save\n\n";
    ...
    while (true) {
        ...
        if (show_flow) {
            ...
            // Draw ego-motion estimate
            cv::Point2f ego = optical_flow.estimate_ego_motion();
            std::ostringstream ego_ss;
            ego_ss << std::fixed << std::setprecision(1)
                   << "Ego: (" << ego.x << ", " << ego.y << ") px/f";
            cv::putText(display, ego_ss.str(),
